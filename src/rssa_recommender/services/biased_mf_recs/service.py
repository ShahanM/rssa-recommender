...
from rssa_recommender.common.mf_base import NumericType, RSSABase, normalize
...
class BiasedMFRecsService(RSSABase):
    ...
    def get_candidates(
        ...
        candidates: pd.DataFrame = cast(pd.DataFrame, candidates[candidates['count'] >= min_rating_count].copy())
    ...
    def calculate_predicted_neighborhood_average(
        ...
        p_nn_ave_df = pd.DataFrame({'p_nn_ave_score': P_NN_Ave_Vector}, index=valid_item_ids)  # type: ignore
    ...
    def _convexhull(self, candidates: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        in the 2D score space (user_score vs. ave_score).vThese items represent the extremes of the model's predictions
        for the user.
        """
    ...
    def _fishingnet(self, candidates: pd.DataFrame, limit: int = 500) -> pd.DataFrame:
        ...
        for point in grid_points:
            ...
            current_distances[is_selected] = np.inf
    ...
    def _single_linkage_clustering(self, candidates: pd.DataFrame, num_clusters: int = 80, **kwargs) -> pd.DataFrame:
        ...
        for i in range(1, num_clusters + 1):
            ...
            if not cluster_df.empty:
                ...
                representative_item = cluster_df.sort_values(by='dist_to_center', ascending=True).iloc[0]  # type:ignore
    ...
    def scale_and_label(self, items: pd.DataFrame, new_min: int = 1, new_max: int = 5) -> pd.DataFrame:
        ...
        if not scaled_items.empty:
            score_min: NumericType = cast(NumericType, scaled_items['score'].min())
            score_max: NumericType = cast(NumericType, scaled_items['score'].max())
            ...
            ave_min: NumericType = cast(NumericType, scaled_items['ave_score'].min())
            ave_max: NumericType = cast(NumericType, scaled_items['ave_score'].max())
